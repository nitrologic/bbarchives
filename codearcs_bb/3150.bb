...
Type ScannerToken
	...
	Function Create:ScannerToken(text$,cat:ScannerCategory=Null,pos_byte%=-1,pos_line%=-1)
		Local n:ScannerToken=New ScannerToken
		n.text=text;n.category=cat
	...
	Method Tokenize:TList(data$,start%=0)
		...
		Repeat
			...
			If bestcat And bestlength ' token has been neatly categorized
				If bestcat.significant token=ScannerToken.Create(beststr,bestcat,pos,line)
				pos:+bestlength
				line:+CountNewlines(beststr)
			Else ' doesn't fit any category? make it its own token.
				token=ScannerToken.Create(Chr(data[pos]),Null,pos,line)
				pos:+1
