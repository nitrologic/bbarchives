...
; Date: 2003-06-19 12:25:08
; Title: 3D Clock
; Description: 3D Clock in B2D
...
Function RotateClockHandsToArray(Seconds , Minutes , Hours)
	...
	For cnt = 0 To 13
		...
		Clock_Triangles_NORMAL#(cnt + CURRENT6 , 0) = Float Hours_Hand_NORMALS#(cnt , 0) * Cos(RotateAmount) + Hours_Hand_NORMALS#(cnt , 1) * Sin(RotateAmount)
		Clock_Triangles_NORMAL#(cnt + CURRENT6 , 1) = Float Hours_Hand_NORMALS#(cnt , 1) * Cos(RotateAmount) - Hours_Hand_NORMALS#(cnt , 0) * Sin(RotateAmount)
...
Function RemoveObsoleteTriangles(All_Triangles_2_Draw)

	For cnt = 1 To All_Triangles_2_Draw
	
		Flag = 0
		
		A# = Clock_Points_PROJ(Clock_Triangles(Triangles_2_Render(cnt , 0) , 1) , 1)
		B# = Clock_Points_PROJ(Clock_Triangles(Triangles_2_Ren
